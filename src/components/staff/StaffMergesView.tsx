...
export interface StaffMergesProps {
  initialSuggestions?: MergeSuggestion[];
  initialMergePoints?: MergePoint[];
}

export default function StaffMergesView({
  initialSuggestions = [],
  initialMergePoints = [],
}: StaffMergesProps = {}) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900/90 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          {/* Config Controls */}
          <div className="flex flex-wrap items-center gap-3 bg-gray-50 dark:bg-gray-800/60 p-2.5 rounded-2xl border border-gray-200 dark:border-gray-700/80">
            <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
              ...
              <input
                ...
                className="w-16 px-2 py-1 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-xl text-center font-mono text-xs font-bold"
              />
              <span>%</span>
            </div>
              ...
              <div
                ...
                className={`p-4 rounded-2xl border transition-all ${
                  bus.isReadyToDispatch
                    ? "bg-green-50/70 dark:bg-green-950/30 border-green-300 dark:border-green-800/80 shadow-md ring-1 ring-green-500/20"
                    : bus.status === "DISPATCHED"
                    ? "bg-gray-50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-800 opacity-70"
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-[11px] text-gray-500 mt-0.5">{bus.routeName}</div>
                  </div>
      ...
      <div className="bg-white dark:bg-gray-900/90 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100 dark:border-gray-800">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-pink-500 flex items-center gap-1.5">
        ...
        <div className="space-y-3">
          ...
          {pendingSuggestions.length === 0 ? (
            <div className="text-center py-10 border border-dashed border-gray-200 dark:border-gray-800 rounded-2xl text-xs text-gray-400 space-y-1">
              <GitMerge className="w-6 h-6 mx-auto text-gray-400" />
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 rounded-xl bg-pink-500/10 text-pink-600 dark:text-pink-400 border border-pink-500/20 font-mono text-xs font-bold">
                      {item.mergePointCode}
                    </span>
                    <div>
                  ...
                  <span className="px-3 py-1 rounded-full bg-yellow-500/10 text-yellow-600 border border-yellow-500/20 text-xs font-bold uppercase">
                    Awaiting Authority Approval
                  </span>
                </div>

                {/* Transfer Diagram */}
                <div className="grid grid-cols-1 sm:grid-cols-11 items-center gap-3 p-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800">
                  {/* Source Bus (4 cols) */}
                  <div className="sm:col-span-4 p-3 rounded-xl bg-red-50/50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/40">
                    <div className="text-[10px] font-bold uppercase text-red-500">Source Bus (Yielding)</div>
                    <div className="font-black text-sm text-gray-900 dark:text-white mt-0.5">
                      {item.sourceBusName}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      Current Occupancy: <strong className="text-gray-900 dark:text-white">{item.sourceOccupancy} passengers</strong>
                    </div>
                  </div>
                  ...
                  {/* Target Bus (4 cols) */}
                  <div className="sm:col-span-4 p-3 rounded-xl bg-green-50/50 dark:bg-green-950/20 border border-green-200 dark:border-green-900/40">
                    <div className="text-[10px] font-bold uppercase text-green-600 dark:text-green-400">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      New Total: <strong className="text-green-600 dark:text-green-400">{item.combinedOccupancy} / {item.targetCapacity}</strong> ({item.remainingSeats} remaining seats)
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 text-xs font-bold transition-colors"
                  >
                    Reject Merge
                  </button>
                  <button
                    onClick={() => handleApproveMerge(item.id)}
                    className="px-5 py-2 rounded-xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold shadow-md shadow-green-600/20 transition-all active:scale-95"
                  >
      ...
      <div className="bg-white dark:bg-gray-900/90 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="p-2.5 rounded-2xl bg-yellow-50 dark:bg-yellow-950/40 border border-yellow-200 dark:border-yellow-900/50 text-yellow-800 dark:text-yellow-300 text-[11px] font-bold flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {mergePoints.map((mp) => (
            <div
              key={mp.id}
              className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-800 space-y-2"
            >
              ...
              <div className="font-bold text-xs text-gray-900 dark:text-white">{mp.name}</div>
              <div className="text-[11px] text-gray-500">
                Route: <strong>{mp.routeName}</strong>
              </div>
              <div className="text-[11px] text-gray-400">{mp.description || "Designated consolidation point"}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {rejectModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-base text-gray-900 dark:text-white">Reject Merge Suggestion</h3>
            ...
            <form onSubmit={handleSubmitReject} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-xs bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500"
                />
              </div>
