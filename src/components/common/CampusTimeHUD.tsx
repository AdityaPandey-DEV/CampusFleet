    ...
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all shadow-2xs cursor-pointer ${
          isSimulating
            ...
            : "bg-white/90 dark:bg-slate-900/90 hover:bg-slate-50 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200"
        }`}
      ...
      >
        ...
        <span className="relative flex h-2 w-2">
          <span
            className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
              isSimulating ? "bg-amber-400" : "bg-emerald-400"
            }`}
          />
          <span
        ...
        <span className="font-mono font-black text-xs tracking-tight flex items-center gap-1">
          ...
          <span className="text-[9px] uppercase font-bold text-slate-400">IST</span>
        </span>

        {!compact && (
          <>
            <span className="w-px h-3 bg-slate-200 dark:bg-slate-700 hidden sm:inline" />
            <span className="hidden sm:inline-flex items-center gap-1 text-[11px] font-semibold text-slate-600 dark:text-slate-300 truncate max-w-[170px]">
              {activeShift ? (
        ...
        {isSimulating && (
          <span className="text-[9px] font-black uppercase px-1.5 py-0.2 rounded-md bg-amber-200 dark:bg-amber-900 text-amber-900 dark:text-amber-200">
            SIM
          </span>
        )}
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 shadow-2xl z-50 animate-in slide-in-from-top-2 duration-200 space-y-4">
          {/* Header */}
          <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <div className="text-base font-black text-slate-900 dark:text-white flex items-center gap-1.5 mt-0.5">
                <span>{currentTime}</span>
                <span className="text-xs text-slate-400 font-bold">IST (UTC+5:30)</span>
              </div>
            ...
            {isSimulating ? (
              <button
                ...
                className="px-2.5 py-1 rounded-xl bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300 text-xs font-bold flex items-center gap-1 hover:bg-amber-200 transition-colors cursor-pointer"
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2">
                ...
                <button
                  ...
                  className={`p-2 rounded-xl font-bold border transition-all text-center ${
                    currentTime24 === "06:30"
                      ...
                      : "bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 hover:bg-slate-100 text-slate-700 dark:text-slate-200"
                  }`}
                >
                  <div>06:30 AM</div>
              ...
              {isSimulating && (
                <button
                  type="button"
                  onClick={resetSimulatedTime}
                  className="w-full py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-600 dark:text-slate-300 text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-1 cursor-pointer"
                >
                  <RotateCcw className="w-3 h-3" />
