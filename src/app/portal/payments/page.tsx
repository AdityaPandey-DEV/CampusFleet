...
export default function SubscriptionsAndBillingPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in max-w-5xl mx-auto">
      ...
      {/* Subscription Plans Selection - Official GEHU 2026-27 Rates */}
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h3 className="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
              <span>Official Semester Bus Fee List (Session 2026-27)</span>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-extrabold uppercase">
                Director GEHU Bhimtal Approved
              </span>
            </h3>
            <p className="text-xs text-slate-500">
              6-Month Semester transit pass for Graphic Era Hill University (Bhimtal Campus).
            </p>
          </div>
        </div>
          ...
          {plans.map(p => (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">
                    6 Months Semester Pass
                  </span>
                  <span className="text-[10px] font-mono text-slate-400">2026-27</span>
                </div>
                ...
                {/* Stoppages List from Official Document */}
                {p.stoppages && p.stoppages.length > 0 && (
                  <div className="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <span className="text-[10px] uppercase font-bold text-slate-400">
                      Covered Stoppages & Corridors:
                    </span>
                    <div className="flex flex-wrap gap-1">
                      {p.stoppages.map((st, i) => (
                        <span
                          key={i}
                ...
                <ul className="text-xs space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400">
              ...
              >
                ...
                Pay Semester Fee ({formatCurrency(p.price)})
              </button>
            </div>
          ))}
        </div>
