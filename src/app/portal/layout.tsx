...
export default function StudentPortalLayout({
  ...
  const navLinks = [
    { href: "/portal", label: "Overview", icon: BusFront, requiresPayment: true },
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col pb-20 md:pb-0 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map(link => {
              ...
              const isLocked = link.requiresPayment && !isPaymentApproved;
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-2 z-50 animate-in fade-in zoom-in-95">
                    ...
                    {students.map(s => (
                      <button
                        ...
                        className={`w-full text-left p-2.5 rounded-xl text-xs flex items-center justify-between transition-colors ${s.id === activeChildId
      ...
      {/* Main Content Area with Mandatory Payment Gate */}
            ...
            {activeStudent?.paymentStatus === "PENDING_APPROVAL" ? (
              <div className="p-4 bg-amber-50 dark:bg-amber-950/60 rounded-2xl border border-amber-200 dark:border-amber-800 text-left space-y-1">
                ...
                <p className="text-[11px] text-amber-700 dark:text-amber-400">
                  Your payment receipt has been uploaded to Vercel Blob and is queued for verification by the university transport desk.
                </p>
              </div>
            ) : (
        ...
        {navLinks.map(link => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`flex flex-col items-center justify-center gap-1 py-1 px-3 rounded-2xl transition-all relative ${isActive
