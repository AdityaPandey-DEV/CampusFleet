...
export default function StudentOnboardingPage() {
  ...
  const [transitZones, setTransitZones] = useState<TransitZone[]>(() =>
    store.getTransitZones(store.getPrimaryCampus()?.id),
  );
  ...
  const [campusId, setCampusId] = useState(
    () => store.getPrimaryCampus()?.id || "",
  );
  const [campus, setCampus] = useState(
    () => store.getPrimaryCampus()?.name || "Main Campus",
  );
  ...
  const coursesList = useMemo(() => {
    const list = Array.from(
      new Set(classesList.map((c) => c.course).filter(Boolean)),
    ) as string[];
  ...
  const semestersList = useMemo(() => {
    const filtered =
      selectedCourse || department
        ...
        : classesList;
    const list = Array.from(
      new Set(filtered.map((c) => c.semester).filter(Boolean)),
    ) as string[];
  ...
  const handlePhotoFileChange = async (
    ...
    if (isPhotoLocked) {
      alert(
        "Official photo is locked. Only Campus Staff/Admin can update your photo.",
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const targetStudentId =
          activeStudent?.id || `stud-${currentUser?.id || Date.now()}`;
        if (targetStudentId && !targetStudentId.startsWith("stud-")) {
          try {
            await store.updateStudentProfile(targetStudentId, {
              photoUrl: data.url,
            });
            setToast("Photo uploaded and saved! Keep completing your profile.");
            setTimeout(() => setToast(null), 4000);
          } catch (e) {
            /* ignore auto-save fail, they can still hit Submit */
          }
        } else {
          setToast(
            "Photo uploaded successfully! Remember to click Save Profile at the bottom.",
          );
      ...
      fetch("/api/campus")
        ...
        .then((data) => {
          if (data.success && data.campuses && data.campuses.length > 0) {
            ...
            const primary =
              data.primaryCampus ||
              data.campuses.find((c: any) => c.isPrimary) ||
              data.campuses[0];
            if (primary && !campusId) {
  ...
  useEffect(() => {
    ...
    // Auto-populate data
    setFullName(
      (prev) => prev || activeStudent?.fullName || currentUser?.fullName || "",
    );
    setPhone((prev) => prev || activeStudent?.phone || "");
    setCampusId(
      (prev) =>
        prev || activeStudent?.campusId || store.getPrimaryCampus()?.id || "",
    );
    setCampus(
      (prev) =>
        prev ||
        activeStudent?.campus ||
        store.getPrimaryCampus()?.name ||
        "Main Campus",
    );
    ...
    setPrimaryStopId(
      (prev) => prev || activeStudent?.primaryStopId || stops[0]?.id || "",
    );
    setEmergencyName(
      (prev) =>
        prev ||
        (activeStudent?.emergencyContact?.name !== "Campus Desk"
          ? activeStudent?.emergencyContact?.name || ""
          : ""),
    );
    setEmergencyPhone(
      (prev) => prev || activeStudent?.emergencyContact?.phone || "",
    );
    setEmergencyRel(
      (prev) =>
        prev ||
        activeStudent?.emergencyContact?.relationship ||
        "Parent / Guardian",
    );
  ...
  const confirmAndSubmitProfile = async () => {
    ...
    const zoneStops = stops.filter(
      (st) => (st.zoneCode || "ZONE_B") === zoneToConfirm.code,
    );
    const chosenStopId =
      zoneStops.length > 0
        ? zoneStops[0].id
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
            <div className="border-b border-gray-100 dark:border-gray-800 pb-6">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Please complete your student profile. This information is used
                for ID verification, transit zone allocation, and emergency
                contacts.
              </p>
            </div>
                  ...
                  {isPhotoLocked ? (
                    <div className="space-y-2">
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Your official photo has been verified and locked. Only
                        campus transport staff can update this photo.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Upload a clear passport-sized photo. This will be shown
                        to the conductor during boarding.
                      </p>
                      <div className="flex items-center gap-3">
                        <label
                          className={`px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium flex items-center gap-2 transition-colors shadow-sm ${isUploadingPhoto ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                        >
                          {isUploadingPhoto ? (
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className={`w-full text-sm p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:border-blue-500 transition-colors ${
                    isCampusLocked
                      ? "opacity-75 cursor-not-allowed bg-gray-100 dark:bg-gray-850"
                      : ""
                  }`}
                >
                  {campuses.map((c) => (
                ...
                {isZoneLocked ? (
                  <div className="p-4 rounded-xl bg-green-50/80 dark:bg-green-950/40 border border-green-200 dark:border-green-800/80 text-xs text-green-900 dark:text-green-200">
                    ...
                    <p className="text-xs opacity-80 leading-relaxed">
                      Transit zone is locked once semester fee is paid. Only
                      campus transport admin can modify zone allocation after
                      payment.
                    </p>
                  </div>
                ) : (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              <h4
                                className={`text-lg font-black ${isSelected ? "text-blue-700 dark:text-blue-400" : "text-gray-900 dark:text-white"}`}
                              >
                                {z.name}
                              </h4>
                            </div>
        ...
        {zoneToConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div
              ...
              onClick={() => !isSubmitting && setZoneToConfirm(null)}
            />
            <div className="relative bg-white dark:bg-gray-900 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-gray-200 dark:border-gray-800 animate-in zoom-in-95 duration-200">
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  . Are you sure? <br />
                  <br />
                  <span className="text-red-500 font-bold">Important:</span> You
                  cannot change your zone after confirming. You will be
