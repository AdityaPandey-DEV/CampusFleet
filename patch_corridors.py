...
# Add ArrowRight to lucide-react imports if it's missing
if 'ArrowRight' not in text and 'lucide-react' in text:
    text = text.replace('import {', 'import { ArrowRight, ', 1)

# Add next/link import
if 'import Link from "next/link";' not in text:
    text = text.replace('import React', 'import Link from "next/link";\nimport React')

# Find the start of TAB 1
tab1_idx = text.find('{/* ============================================================= */}\n      {/* TAB 1: ROUTES & CORRIDORS                                     */}')

if tab1_idx != -1:
    # Find where the routes tab div ends. It ends exactly before RouteBuilderModal
    end_idx = text.find('      {/* Route Builder & Allocation Modals */}')
                  ...
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col h-full hover:border-blue-300 dark:hover:border-blue-700/50">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6 mt-auto">
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-3 border border-gray-100 dark:border-gray-800">
                         ...
                         <span className="font-black text-gray-900 dark:text-white text-sm">{r.distanceKm} <span className="text-gray-400 font-normal">km</span></span>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-3 border border-gray-100 dark:border-gray-800">
                         <span className="text-[10px] text-gray-500 font-bold uppercase block mb-0.5">Duration</span>
                         <span className="font-black text-gray-900 dark:text-white text-sm">~{r.estimatedDurationMins} <span className="text-gray-400 font-normal">min</span></span>
                      </div>
                    </div>
...
"""
        text = text[:tab1_idx] + new_content + text[end_idx:]
