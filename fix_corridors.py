...
start_idx = text.find('{/* CORRIDORS GRID OR CREATE VIEW                                 */}')
if start_idx == -1:
    print("Start string not found")

# We want to replace everything from `      {mode === "create" ? (` to `      {/* Route Builder & Allocation Modals */}`
import re
pattern = r'\{mode === "create" \? \([\s\S]*?\{/\* Route Builder & Allocation Modals \*/\}'
...
if match:
    new_code = """{mode === "create" ? (
      ...
      {/* Route Builder & Allocation Modals */}"""
