...
# Find the start of the old div
start_line = -1
for i, line in enumerate(lines):
    if '<div className="space-y-6 pt-2">' in line:
        start_line = i
        break

# Find the start of AllocateBusModal
end_line = -1
for i, line in enumerate(lines):
    if '<AllocateBusModal' in line:
        end_line = i
        break

if start_line != -1 and end_line != -1:
    new_content = """        <div className="space-y-6 pt-2">
          {routes.length > 0 ? (
